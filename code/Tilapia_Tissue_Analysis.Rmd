...
colors11 <- c("#650111", "#de8998", "#795015", "#eebd78", "#035c00", "#b4e683", "#082674", "#9ecee8", "#891989", "#d9b1ee", "#343434" )
...
colors_H1_like_9589 <- colors11[c(2,4,5,8,9,10,11)]
colors_H2A_8939 <- colors11[c(2,3,5,6,7,8,9,10,11)]
colors_H2B_1_2 <- colors11[c(2,4,5,6,7,8,9,10,11)]
colors_H3 <- colors11[c(2,5,6,7,8,9,10,11)]
colors_H4_like <- colors11[c(2,5,7,8,9,10,11)]
```
...
length(unique(All_Ambic$hPTM_Name)) # 129
...
length(unique(All_Napho$hPTM_Name)) # 124
...
length(unique(All_Trypsin$hPTM_Name)) # 33
...
length(unique(Exp2_Data$hPTM_Name)) # 196 biologically relevant histone PTMs that display variance of M values in total
...
num_napho_only <- length(unique(Napho_only$hPTM_Name)) # 42 biologically relevant histone PTMs detected only using the V8 Napho digestion condition

num_ambic_only <- length(unique(Ambic_only$hPTM_Name)) # 46 biologically relevant histone PTMs detected only using the V8 Ambic digestion condition

num_trypsin_only <- length(unique(Trypsin_only$hPTM_Name)) # 24 biologically relevant histone PTMs detected only using the trypsin/propionylation digestion condition

num_ambic_and_napho <- length(unique(Ambic_and_Napho$hPTM_Name)) # 75 biologically relevant histone PTMs detected using either the V8 Ambic or V8 Napho digestion conditions
...
Total_Napho_hPTMs <- num_napho_only + num_ambic_and_napho + num_napho_and_trypsin + num_ambic_napho_trypsin # 124 biologically relevant histone PTMs can be detected using the V8 napho digestion method

Total_Ambic_hPTMs <- num_ambic_only + num_ambic_and_napho + num_trypsin_and_ambic + num_ambic_napho_trypsin # 129 biologically relevant histone PTMs can be detected using the V8 ambic digestion method

Total_Trypsin_hPTMs <- num_trypsin_only + num_trypsin_and_ambic + num_napho_and_trypsin + num_ambic_napho_trypsin # 33 biologically relevant histone PTMs can be detected using the trypsin/propionylation digestion method
...
rep_str = c("XP_003448939.1 H2A P48 Dioxidation" = "H2A P48 Dioxidation", 
            ...
            "XP_003459572.1 H2A K123 Acetyl" = "H2A K123 Acetyl",
